test_that("Test that input argument checks work", {
  ...
  expect_error(intensity_levels(tsd_data, decay_factor = 3), "Variable 'decay_factor': Element 1 is not <= 1.")

  expect_error(intensity_levels(tsd_data, n_peak = c(1, 2)), "Variable 'n_peak': Must have length 1, but has length 2.")

  expect_error(checkmate_err_msg(intensity_levels(tsd_data, disease_threshold = "hey"),
               "'disease_threshold': Must be of type 'integerish', not 'character'.",
               fixed = TRUE))

  expect_error(intensity_levels(tsd_data, season_weeks = c(2, 10)), "`start` must be greater than `end`!")
