...
test_that("Test that function fail with less than two seasons", {
  ...
  withr::with_seed(
    ...
    code = {
      # Generate seasonal data
      tsd_one_season <- generate_seasonal_data(years = 1, start_date = as.Date("2021-05-24"))
...
test_that("Test that function does not fail if there are no observations surpassing the disease specific threshold
    ...
    withr::with_seed(
      ...
      code = {
        # Generate seasonal data
        tsd_data_one <- generate_seasonal_data(years = 1, start_date = as.Date("2022-05-23"))
        tsd_data_two <- generate_seasonal_data(years = 1, amplitude = 10, start_date = as.Date("2023-05-22"))
        tsd_data_combined <- dplyr::bind_rows(tsd_data_one, tsd_data_two)

        burden_list <- seasonal_burden_levels(tsd_data_combined, disease_threshold = 1000)
