...
\begin{equation}
  SeasonalWave(t) = [\text{amplitude} * (1 + \sin(\frac{2\pi t}{\text{period}} + \text{phase}))] * e^{\text{log(trend\_rate)} * t} + \epsilon
\end{equation}

Where:
- $t$: The time variable (e.g., weeks or months, represented on the x-axis).
- $\text{amplitude}$: Controls the height of the wave; the output oscillates between $0 to 2 * \text{amplitude}$.
- $\text{period}$: Defines the cycle length (e.g., 52 weeks for yearly seasonality).
- $\text{phase}$: Adjusts the horizontal position of the wave on the x-axis.
- $\text{trend\_rate}$: Controls the exponential growth or decay of the trend over time.
...
```{r}
seasonal_wave_sim_weekly <- generate_seasonal_data(years = 3,
...
The following figure shows the simulated data (solid circles) as individual observations.
...
```{r}
seasonal_wave_sim_monthly <- generate_seasonal_data(years = 3,
                                                    start_date = as.Date("2021-05-26"),
                                                    amplitude = 1000,
                                                    phase = 0,
                                                    trend_rate = 1.001,
...
plot(seasonal_wave_sim_monthly, time_interval = "2 months")
```

Example of daily observations
```{r}
seasonal_wave_sim_daily <- generate_seasonal_data(years = 1,
                                                  start_date = as.Date("2021-05-26"),
                                                  amplitude = 1000,
                                                  phase = 0,
                                                  trend_rate = 1.001,
