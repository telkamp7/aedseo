...
disease-specific threshold, while also evaluating the burden of current observations based on previous seasons.
...
The `seasonal_onset()` function can be used for this purpose, without providing the disease threshold.
```{r}
...
The `aedseo` package implements S3 methods including the `plot()`, `predict()` and `summary()` functions. `predict` is only relevant for `tsd_onset` objects.
