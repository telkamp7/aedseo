...
tsd_data_pos_trend <- generate_seasonal_data(
  years = 4,
  start_date = as.Date("2021-10-18"),
  amplitude = 2000,
  phase = 0,
...
tsd_data_neg_trend <- generate_seasonal_data(
  years = 4,
  start_date = as.Date("2021-10-18"),
  amplitude = 2000,
  phase = 0,
...
tsd_data_pos_trend <- tsd_data_pos_trend |>
  dplyr::filter(time <= "2025-05-12")
tsd_data_noise_and_pos_trend <- tsd_data_noise_and_pos_trend |>
  dplyr::filter(time <= "2025-05-12")
tsd_data_neg_trend <- tsd_data_neg_trend |>
  dplyr::filter(time <= "2025-05-12")
...
intensity_levels_pos_t <- seasonal_burden_levels(
  tsd = tsd_data_pos_trend,
  disease_threshold = 140,
  method = "intensity_levels",
  conf_levels = 0.95
)

intensity_levels_neg_t <- seasonal_burden_levels(
  tsd = tsd_data_neg_trend,
  disease_threshold = 140,
  method = "intensity_levels",
  conf_levels = 0.95
)
...
peak_levels_pos_t <- seasonal_burden_levels(
  tsd = tsd_data_pos_trend,
  disease_threshold = 140,
  method = "peak_levels",
  conf_levels = c(0.4, 0.9, 0.975),
...
peak_levels_neg_t <- seasonal_burden_levels(
  tsd = tsd_data_neg_trend,
  disease_threshold = 140,
  method = "peak_levels",
  conf_levels = c(0.4, 0.9, 0.975),
...
### Aedseo levels
```{r, echo = FALSE, fig.width=10, fig.height=9, dpi=300}
# Create data frames
burden_levels_df <- tibble::tibble(
  Level = names(
    c(intensity_levels$values,
      intensity_levels_n$values,
      intensity_levels_pos_t$values,
      intensity_levels_neg_t$values,
      intensity_levels_n_pos_t$values,
      intensity_levels_n_neg_t$values,
      peak_levels$values,
      peak_levels_n$values,
      peak_levels_pos_t$values,
      peak_levels_neg_t$values,
...
mem_levels_df <- tibble::tibble(
  Level = names(
    c(mem_thresholds$tsd_data_clean,
      mem_thresholds$tsd_data_noise,
      mem_thresholds$tsd_data_pos_trend,
      mem_thresholds$tsd_data_neg_trend,
      mem_thresholds$tsd_data_noise_and_pos_trend,
      mem_thresholds$tsd_data_noise_and_neg_trend
    )
  ),
  Threshold = c(
    as.numeric(mem_thresholds$tsd_data_clean),
    as.numeric(mem_thresholds$tsd_data_noise),
    as.numeric(mem_thresholds$tsd_data_pos_trend),
    as.numeric(mem_thresholds$tsd_data_neg_trend),
